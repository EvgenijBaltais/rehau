<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Программа «СПК»</title>
	<link rel="stylesheet" href="../css/style.css">
</head>
<body>
	<div class="wrapper">
		<? require_once('../php/header.php'); ?>
		<div class="top-slider top-slider-spk" style = "background-image: url('/images/g.jpg');">
			<div class="slider-block">
				<p class = "slider-block-title">Программа «СПК»</p>
				<button class = "slider-block-button">Перейти к программе</button>
			</div>
		</div>
		<section class = "section page-content">
			<h1 class = "h1">«Светопрозрачные конструкции»</h1>
			<div class = "page-table">
				<table class = "table">
					<thead>
						<tr>
							<td class = "thead-td">День</td>
							<td class = "thead-td">Время</td>
							<td class = "thead-td">Содержание</td>
							<td class = "thead-td">Референт</td>
							<td class = "thead-td">Зал</td>
						</tr>
					</thead>
					<tbody>
<!-- Первый ряд -->
						<tr>
							<td rowspan = "5" valign = "top" align = "center" class = "colored-td bold-td day-td">
								ПН<br>
								26.02
							</td>
							<td class = "time-td">
								14:00-15:30
							</td>
							<td>
								Качество РЕХАУ. Сертификация
							</td>
							<td class = "name-td">Константин Гайнуллин,
								Антон Карявкин
							</td>
							<td class = "hall-td" rowspan = "5" valign = "top">
								Мехико
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								15:30-16:00
							</td>
							<td colspan = "2" class = "colored-td">
								КОФЕ-БРЕЙК
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								16:00-17:00
							</td>
							<td>
								Развитие рынка. Итоги 2023 года
							</td>
							<td class = "name-td">
								Павел Иваненко
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								17:00-18:00
							</td>
							<td>
								Итоги бюро 2023. Цели 2024. SIP 2024. Новая годовая цель SP.
							</td>
							<td class = "name-td">
								Павел Калаущенко
							</td>
						</tr>
						<tr>
							<td class = "empty-td"></td>
							<td class = "empty-td"></td>
							<td class = "empty-td"></td>
						</tr>
<!-- Первый ряд, конец -->
<!-- Второй ряд -->
						<tr>
							<td rowspan = "13" valign = "top" align = "center" class = "colored-td bold-td day-td">
								ВТ<br>
								27.02
							</td>
							<td class = "time-td">
								09:00-10:00
							</td>
							<td>
								Регламент по сбыту
							</td>
							<td class = "name-td">
								Павел Калаущенко
							</td>
							<td rowspan = "13" valign = "top" class = "hall-td">
								Мехико
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								10:00-11:00
							</td>
							<td>
								Продуктовая политика: развитие и локализация. Калькулятор раздвижных систем.
							</td>
							<td class = "name-td">
								Ольга Царькова
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								11:00-11:15
							</td>
							<td colspan = "2" class = "colored-td">
								КОФЕ-БРЕЙК
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								11:15-12:15
							</td>
							<td>
								Продуктовая политика: алюминиевые системы RESOLUT
							</td>
							<td class = "name-td">
								Илья Бендерин
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								12:15-13:00
							</td>
							<td>
								Цветовая палитра РЕХАУ 2024
							</td>
							<td class = "name-td">
								Людмила Багдасарян
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								13:00-14:00
							</td>
							<td colspan = "2" class = "colored-td">
								ОБЕД
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								14:00-14:45
							</td>
							<td>
								Трейд Маркетинг: итоги 2023 и планы 2024
							</td>
							<td class = "name-td">
								Вячеслав Рыблов
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								14:45-15:45
							</td>
							<td>
								Маркетплейсы
							</td>
							<td class = "name-td">
								Андрей Куц
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								15:45-16:00
							</td>
							<td colspan = "2" class = "colored-td">
								КОФЕ-БРЕЙК
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								16:00-17:00
							</td>
							<td>
								Объектный бизнес. Сотрудничество с Roto Frank
							</td>
							<td class = "name-td">
								Руслан Тюменев
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								17:00-18:00
							</td>
							<td>
								Презентация о цифровых проектах
							</td>
							<td class = "name-td">
								Дмитрий Брацун
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								19.00-21.00
							</td>
							<td class = "colored-td">
								для зарегистрировавшихся: экскурсия «Вечерний город» (автобус 1) или «Мистический Санкт-Петербург» (автобус 2)
							</td>
							<td class = "colored-td">
								Встреча в лобби в 18.45
							</td>
						</tr>
						<tr>
							<td class = "empty-td"></td>
							<td class = "empty-td"></td>
							<td class = "empty-td"></td>
						</tr>
<!-- Второй ряд, конец -->
<!-- Третий ряд -->
						<tr>
							<td rowspan = "9" valign = "top" align = "center" class = "colored-td bold-td day-td">
								СР<br>
								28.02
							</td>
							<td class = "time-td">
								08:30-09:00
							</td>
							<td>
								Производственная стратегия и запасы
							</td>
							<td class = "name-td">
								Полина Симарзина
							</td>
							<td rowspan = "9" valign = "top" class = "hall-td">
								Мехико
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								09:00-10:30
							</td>
							<td>
								Итоги РК 2023. Исследование потребителей. Медиаплан 2024.
							</td>
							<td class = "name-td">
								Анастасия Бычкова
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								10:30-10:45
							</td>
							<td colspan = "2" class = "colored-td">
								КОФЕ-БРЕЙК
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								10:45-11:00
							</td>
							<td>
								Академия: знакомство
							</td>
							<td class = "name-td">
								Виктория Соколова
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								11:00-13:00
							</td>
							<td>
								Выступления VTLs
							</td>
							<td>
								VTLs
							</td>
						</tr>
						<tr>
							<td class = "colored-td time-td">
								13:00-14:00
							</td>
							<td colspan = "2" class = "colored-td">
								ОБЕД
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								14:15-15:00
							</td>
							<td>
								ERP 1C
							</td>
							<td class = "name-td">
								Наталия Стрекаловская
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								15:00-16:00
							</td>
							<td>
								Тренды на рынке оконных комплектующих
							</td>
							<td class = "name-td">
								Дмитрий Пацей
							</td>
						</tr>
						<tr>
							<td class = "time-td">
								16:00-16:45
							</td>
							<td>
								CRM
							</td>
							<td class = "name-td">
								Дмитрий Брацун
							</td>
						</tr>
<!-- Третий ряд, конец -->
					</tbody>
				</table>
			</div>
		</section>
        <? require_once('../php/to-top.php'); ?>
        <? require_once('../php/green-bottom.php'); ?>
	</div>
	<? require_once('../php/footer.php'); ?>
	<script src = "/js/script.js"></script>
</body>
</html>